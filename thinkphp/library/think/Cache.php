...
class Cache
{
     ...
     * @return Driver
     */
    public static function redisinit(array $options = [])
    {
        if (is_null(self::$handler)) {
            if (empty($options) && 'complex' == Config::get('cache.type')) {
                $default = Config::get('cache.redis');
                // 获取默认缓存配置，并连接
                $options = Config::get('cache.' . $default['type']) ?: $default;
            } elseif (empty($options)) {
                $options = Config::get('cache');
    ...
    {
        return self::redisinit()->handler()->Llen($name);
    }
     ...
     * Date: 2019/1/21
     */
    public static function rpush($name, $data = null)
    {
        return self::redisinit()->handler()->rpush($name, $data);
    }
     ...
     * @access public
     * @param string        $name  队列名
     * @param int           $start 指定开始位置
     * @param int           $end   指定结束位置
     * @return mixed
     * User: HeGuoxiang
     * Date: 2019/1/21
     */
    public static function lrange($name = null, $start = 0, $end = 0)
    {
        return self::redisinit()->handler()->lrange($name, $start, $end);
    }
     ...
     * @param string        $value 队列元素
     * @return mixed
     * User: HeGuoxiang
     * Date: 2019/1/21
     */
    public static function lrem($name, $value, $count = 1)
    {
        return self::redisinit()->handler()->lrem($name, $value, $count);
    }
}
